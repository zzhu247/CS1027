...
public class CellComponent extends JComponent  {
	...
	private String imgHero = "adventurer.jpg";
	...
	private String imgCandy = "candy.jpg";
	private String imgCandyPushed = "candy_pushed.jpg";
	private String imgCandyPopped = "candy_popped.jpg";
	private String imgZombie = "zombie1.jpg";
	private String imgSuperHero = "superhero.jpg";
	...
	protected void paintComponent(Graphics g) {
		...
		if (c == CellColours.BLOCK) {
		...
		} else if (c == CellColours.CANDY_PROCESSED || c == CellColours.HERO_PROCESSED
				|| c == CellColours.HERO_POPPED || c == CellColours.CANDY_POPPED || c == CellColours.HERO) {
			// Initial and destination cells when they have been reached
			displayed = true;
			Image img;
			if (c == CellColours.CANDY_PROCESSED || c == CellColours.HERO_PROCESSED)
			...
			g2d.setPaint(gp);
			g2d.fillPolygon(cellPoly);
			if (c == CellColours.CANDY_PROCESSED) {
				img = new ImageIcon(imgCandyPushed).getImage();
				...
				g2d.drawImage(img, 0, 0, width, width, null);
			} else if (c == CellColours.CANDY_POPPED) {
				img = new ImageIcon(imgCandyPopped).getImage();
				g2d.drawImage(img,  0,  0,  width,  width,  null);
			} else if (c== CellColours.HERO) {
				img = new ImageIcon(imgHero).getImage();
				//g2d.drawImage(img, width / scale, height / scale, (scale-2) * width / scale, (scale-2) * height / scale, null);
				g2d.drawImage(img, 0, 0, width, width, null);
			} else {
			...
			g2d.setPaint(gp);
			g2d.fillPolygon(cellPoly);
			img = new ImageIcon(imgCandy).getImage();
			//g2d.drawImage(img, width / 5, height / 5, 3 * width / 5, 3 * height / 5, null);
			g2d.drawImage(img, 0, 0, width, width, null);

			displayed = true;
		} else if (c == CellColours.HORIZ_PATH || c == CellColours.HORIZ_PATH_PUSHED || c == CellColours.HORIZ_PATH_POPPED) {
			// ODD_SWITCH cells: iniital state, when they are visited for the first
			// time,
		...
		else if (c == CellColours.ZOMBIE) {
			Image img;
			if(c == CellColours.ZOMBIE) {
				g2d.setColor(new Color(0,0,0));
				g2d.fillPolygon(cellPoly);
				img = new ImageIcon(imgZombie).getImage();
			}
			else {
				g2d.setColor(new Color(0,0,0));
				g2d.fillPolygon(cellPoly);
		...
		else if (c == CellColours.SUPERHERO) {
			Image img;
			g2d.setColor(new Color(250,250,250));
			g2d.fillPolygon(cellPoly);
			img = new ImageIcon(imgSuperHero).getImage();
			g2d.drawImage(img, 0, 0, width, width, null);
		}
		/*
		if (!displayed) {
